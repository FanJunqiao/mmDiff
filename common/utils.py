...
def save_prediction_3d(pred_tensor, true_tensor, epochs, mpj, test_scene = "Lab1"):
    ...
    fig, axes = plt.subplots(nrows = 1, ncols = 2, subplot_kw=dict(projection='3d'))
    fig.set_figwidth(10)
    axes[0].scatter(true[:, 0], true[:, 1], true[:, 2],marker="o")
    # axes[0].set_xlabel('Human Pose True', labelpad=5)
    axes[1].scatter(pred[:, 0], pred[:, 1], pred[:, 2], marker="o")
    # axes[1].set_xlabel('Human Pose Pred', labelpad=5)
    ...
    for i, j in lines:
        color += 0.9 / len(lines)
        axes[0].plot([true[i][0], true[j][0]], [true[i][1], true[j][1]], [true[i][2], true[j][2]],
                     c=plt.cm.gist_ncar(color))
    color = 0.0
    for i, j in lines:
        color += 0.9 / len(lines)
        axes[1].plot([pred[i][0], pred[j][0]], [pred[i][1], pred[j][1]], [pred[i][2], pred[j][2]],
                     c=plt.cm.gist_ncar(color))

    if test_scene == "Lab1":
        axes[0].set_xlim([-0.6, 0.6])
    axes[0].set_ylim([-0.6, 0.6])
    axes[0].set_zlim([-0.6, 0.6])
    if test_scene == "Lab1":
        axes[1].set_xlim([-0.6, 0.6])
    axes[1].set_ylim([-0.6, 0.6])
    axes[1].set_zlim([-0.6, 0.6])
    ...
    for a in axes:
        ...
        a.set_yticklabels([])
        a.set_zticklabels([])
    # plt.title(f"MPJPE = {mpj}")

    plt.savefig(os.path.join("test_plot", f"prediction_{epochs}.png"))
    plt.close()

def save_prediction(pred_tensor, true_tensor, epochs, mpj, test_scene = "Lab1"):
