...
from pointnet2_utils_torch import query_ball_point, farthest_point_sample, index_points, sample_and_group, sample_and_group_all, index_points_ball_query, index_points_gather
...
class P4DConv(nn.Module):
    ...
    def forward(self, xyzs: torch.Tensor, features: torch.Tensor = None) -> (torch.Tensor, torch.Tensor):
        ...
        for t in range(self.temporal_kernel_size//2, len(xyzs)-self.temporal_kernel_size//2, self.temporal_stride):                 # temporal anchor frames
            ...
            anchor_idx = farthest_point_sample(xyzs[t], npoints//self.spatial_stride)

            anchor_xyz = index_points_gather(xyzs[t], anchor_idx)

            anchor_xyz_flipped = anchor_xyz.transpose(1, 2).contiguous() 
            ...
            for i in range(t-self.temporal_kernel_size//2, t+self.temporal_kernel_size//2+1):
                ...
                neighbor_feature = features[i].permute(0,2,1)
                # print(neighbor_feature.shape)
                

                idx = query_ball_point(self.r, self.k, neighbor_xyz, anchor_xyz).to(torch.int32)
                # print(idx.shape)
                # neighbor_xyz_grouped = index_points_ball_query(neighbor_xyz, idx)
                # print(neighbor_xyz_grouped.dtype)
                # neighbor_xyz_grouped = neighbor_xyz_grouped.permute(0,3,1,2)
                ...
                del xyz_displacement, t_displacement, neighbor_xyz_grouped
                # print(displacement.size())
                displacement = self.conv_d(displacement)
                # print("hi", i, t, self.temporal_kernel_size)

                if self.in_planes != 0:
                    # neighbor_feature_grouped = index_points(neighbor_feature, idx).permute(0,3,1,2).contiguous()
                    neighbor_feature_grouped = pointnet2_utils.grouping_operation(features[i], idx)                                 # (B, in_planes, N//spatial_stride, k)
                    # print("feat", neighbor_feature_grouped.shape)
                    feature = self.conv_f(neighbor_feature_grouped)
                    del neighbor_feature_grouped
                    if self.operator == '+':
