...
class P4Transformer_feat(nn.Module):
    ...
    def forward(self, radar):    
        ...
        joint_embedding =self.joint_posembeds_vector.expand(radar.size()[0], -1, -1) #  torch.rand(size = (embedding.size()[0], self.joint_num, embedding.size()[2])).cuda() +         
